...
async function testTaxiResult() {
    ...
    console.log('Проверка условия тест-кейса');
        if (text.includes('94 руб') && text.includes('10 мин')) {
fs.writeFile('result-t-2.txt', 'Test passed', (err) => {
    if (err) throw err;
    console.log('Passed. Результат записан в файл "result-t-2.txt"');
});
        
    } else {
          fs.writeFile('result-t-2.txt', 'Test failed. Actual result: ' + text, (err) => {
    if (err) throw err;
    console.log('Failed. Результат записан в файл "result-t-2.txt"');
});
    }
