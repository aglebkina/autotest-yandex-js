...
async function testTaxiResult() {
    ...
    await toInput.type('Фрунзенская набережная, 46');
    ...
    const typeOfTransport = await page.$('#from-type-car');
    ...
    console.log('Проверка условия тест-кейса');
        if (text.includes('Бесплатно') && text.includes('0 мин')) {
fs.writeFile('result-t-3.txt', 'Test passed', (err) => {
    if (err) throw err;
    console.log('Passed. Результат записан в файл "result-t-3.txt"');
});
        
    } else {
          fs.writeFile('result-t-3.txt', 'Test failed. Actual result: ' + text, (err) => {
    if (err) throw err;
    console.log('Failed. Результат записан в файл "result-t-3.txt"');
});
    }
