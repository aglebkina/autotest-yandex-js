...
async function testTaxiResult() {
    ...
    const typeOfTransport = await page.$('#from-type-bike');
    ...
    console.log('Проверка условия тест-кейса');
        if (text.includes('14 руб') && text.includes('24 мин')) {
fs.writeFile('result-t-4.txt', 'Test passed', (err) => {
    if (err) throw err;
    console.log('Passed. Результат записан в файл "result-t-4.txt"');
});
        
    } else {
          fs.writeFile('result-t-4.txt', 'Test failed. Actual result: ' + text, (err) => {
    if (err) throw err;
    console.log('Failed. Результат записан в файл "result-t-4.txt"');
});
    }
