...
async function testTaxiResult() {
    ...
    console.log('Проверка условия тест-кейса');
        if (text.includes('51 руб') && text.includes('5 мин')) {
fs.writeFile('result-t-6.txt', 'Test passed', (err) => {
    if (err) throw err;
    console.log('Passed. Результат записан в файл "result-t-6.txt"');
});
        
    } else {
          fs.writeFile('result-t-6.txt', 'Test failed. Actual result: ' + text, (err) => {
    if (err) throw err;
    console.log('Failed. Результат записан в файл "result-t-6.txt"');
});
    }
